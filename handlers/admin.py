...
from .sqlit import members_list, change_bot
...
async def admin_ka(message: types.Message):
    if message.from_user.id in ADMIN_ID:
        change_bot('1')
        await message.answer('Включен режим привата')


@dp.message_handler(commands=['intim'])
async def admin_ka(message: types.Message):
    if message.from_user.id in ADMIN_ID:
        change_bot('0')
        await message.answer('Включен режим интима')
